$(document).ready(function () {
    ...
    var ajaxurl = 'app/core/assets/install/ajax';
    ...
    window.validarConexionDB = function () {
        ...
        $.post(ajaxurl + '/validar-conexion-db.php', data).then(function (res) {
            if (res.error) {
                ...
                $stepper.find('input[name="db[user]"]').showError(res.message);
            } else {
                $stepper.nextStep();
            }
        }).fail(function () {
    ...
    $form.submit(function (e) {
        ...
        $.post(ajaxurl + '/instalar.php', data).then(function (res) {
            if (res.error || !res.url) {
                $stepper.destroyFeedback();
                $stepper.find('input[name="admin[correo]"]').showError(res.message);
            } else {
