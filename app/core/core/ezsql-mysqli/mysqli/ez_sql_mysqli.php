...
class ezSQL_mysqli extends ezSQLcore
{
	 ...
	 *
	 *     lrvdb::prepare( "SELECT * FROM `table` WHERE `column` = %s AND `field` = %d", 'foo', 1337 )
	 *     lrvdb::prepare( "SELECT DATE_FORMAT(`field`, '%%c') FROM `table` WHERE `column` = %s", 'foo' );
	 *
	...
	/* Funciones tomadas del lrvdb -> (insert, update, delete) */

	/**
	 * Insert a row into a table.
	 *
	 *     lrvdb::insert( 'table', array( 'column' => 'foo', 'field' => 'bar' ) )
	 *     lrvdb::insert( 'table', array( 'column' => 'foo', 'field' => 1337 ), array( '%s', '%d' ) )
	 *
	 * @since 2.5.0
	 * @see lrvdb::prepare()
	 * @see lrvdb::$field_types
	 *
	 ...
	 *                             If omitted, all values in $data will be treated as strings unless otherwise specified in lrvdb::$field_types.
	 ...
	 *
	 *     lrvdb::replace( 'table', array( 'column' => 'foo', 'field' => 'bar' ) )
	 *     lrvdb::replace( 'table', array( 'column' => 'foo', 'field' => 1337 ), array( '%s', '%d' ) )
	 *
	 * @since 3.0.0
	 * @see lrvdb::prepare()
	 * @see lrvdb::$field_types
	 ...
	 *
	 *     lrvdb::update( 'table', array( 'column' => 'foo', 'field' => 'bar' ), array( 'ID' => 1 ) )
	 *     lrvdb::update( 'table', array( 'column' => 'foo', 'field' => 1337 ), array( 'ID' => 1 ), array( '%s', '%d' ), array( '%d' ) )
	 *
	 * @since 2.5.0
	 * @see lrvdb::prepare()
	 * @see lrvdb::$field_types
	 ...
	 *
	 *     lrvdb::delete( 'table', array( 'ID' => 1 ) )
	 *     lrvdb::delete( 'table', array( 'ID' => 1 ), array( '%d' ) )
	 *
	 * @since 3.4.0
	 * @see lrvdb::prepare()
	 * @see lrvdb::$field_types
	 ...
	 *                                   If string, that format will be used for all of the items in $where.
	 *                                   A format is one of '%d', '%f', '%s' (integer, float, string).
	 *                                   If omitted, all values in $where will be treated as strings unless otherwise specified in lrvdb::$field_types.
	 * @return int|false The number of rows updated, or false on error.
	 */
	 ...
	 *
	 * This is a helper method for lrvdb's CRUD methods, which take field/value
	...
	/**
	 * Prepares arrays of value/format pairs as passed to lrvdb CRUD methods.
	 *
	 * @since 4.2.0
	 * @access protected
	 *
	 ...
	 * the lrvdb::process_field_formats() method.
	 *
	 * @since 4.2.0
	 * @access protected
	 *
	 * @param array  $data  As it comes from the lrvdb::process_field_formats() method.
	 * @param string $table Table name.
	 ...
	 * @access protected
	 *
	 * @param array  $data  As it comes from the lrvdb::process_field_charsets() method.
	 * @param string $table Table name.
	...
	protected function get_table_charset($table)
	{
		...
		if (!$results) {
			return new Exception('lrvdb_get_table_charset_failure');
		}
	...
	protected function strip_invalid_text($data)
	{
		...
		if ($db_check_string) {
			...
			if (!$row) {
				return new Exception('lrvdb_strip_invalid_text_failure');
			}
