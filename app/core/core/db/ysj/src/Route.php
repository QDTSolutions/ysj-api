...
class Route {
	...
	public function __construct($args) {
		...
		$this->_caseSensitive = (array_key_exists('caseSensitive', $args) && $args['caseSensitive']) ? true : false;
		$this->_view = (array_key_exists('view', $args) && $args['view']) ? true : false;

		$args['route'] = !empty($args['route']) ? $args['route'] : '';

		if (!@preg_match($args['route'], null) && (strlen($args['route']) && $args['route'][0] == '/')) {
